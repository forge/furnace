...
import org.jboss.forge.furnace.impl.util.visitors.DeleteVisitor;
...
public final class Files
{
   ...
   public static boolean delete(File file, final boolean recursive) throws IOException
   {
      Assert.notNull(file, "File to delete must not be null.");
      ...
      if (recursive)
      {
         try
         {
            java.nio.file.Files.walkFileTree(file.toPath(), new DeleteVisitor());
            result = true;
         }
         ...
         {
            throw new IOException("Failed to delete recursive '" + file + "' Cause: " + e.getMessage());
         }
      }
      else
      {
